...
func CheckNodeThresholds(metrics *model.NodeMetrics) []*model.AlertEvent {
	...
	if metrics.Status != "online" {
		alerts = append(alerts, &model.AlertEvent{
			...
			Type:        "NodeOffline",
			Severity:    model.SeverityCritical,
			Source:      fmt.Sprintf("Node-%s", metrics.ID),
	...
	if cpuUsage, ok := metrics.CPUUsage.(float64); ok {
		if cpuUsage > 85.0 {
			alerts = append(alerts, &model.AlertEvent{
				...
				Type:        "HighCPUUsage",
				Severity:    model.SeverityCritical,
				Source:      fmt.Sprintf("Node-%s-CPU", metrics.ID),
	...
	if metrics.MemoryTotal > 0 {
		...
		if memoryPercent > 90.0 {
			alerts = append(alerts, &model.AlertEvent{
				...
				Severity:    model.SeverityCritical,
				Source:      fmt.Sprintf("Node-%s-Memory", metrics.ID),
	...
	if metrics.DiskTotal > 0 {
		...
		if diskPercent > 90.0 {
			alerts = append(alerts, &model.AlertEvent{
				...
				Type:        "HighDiskUsage",
				Severity:    model.SeverityCritical,
				Source:      fmt.Sprintf("Node-%s-Disk", metrics.ID),
...
func CheckContainerThresholds(metrics *model.ContainerMetrics) []*model.AlertEvent {
	...
	if metrics.CPUUsage.Total > 65.0 {
		alerts = append(alerts, &model.AlertEvent{
			...
			Type:        "HighCPUUsage",
			Severity:    model.SeverityCritical,
			Source:      fmt.Sprintf("Container-%s-CPU", metrics.ID),
	...
	if metrics.MemoryLimit > 0 {
		...
		if memoryPercent > 80.0 {
			alerts = append(alerts, &model.AlertEvent{
				...
				Type:        "HighMemoryUsage",
				Severity:    model.SeverityCritical,
				Source:      fmt.Sprintf("Container-%s-Memory", metrics.ID),
	...
	if metrics.SizeLimit > 0 {
		...
		if diskPercent > 65.0 {
			alerts = append(alerts, &model.AlertEvent{
				...
				Type:        "HighDiskUsage",
				Severity:    model.SeverityCritical,
				Source:      fmt.Sprintf("Container-%s-Disk", metrics.ID),
...
func CheckServiceThresholds(metrics *model.ServiceMetrics) []*model.AlertEvent {
	...
	if !metrics.Healthy {
		alerts = append(alerts, &model.AlertEvent{
			...
			Severity:    model.SeverityWarning,
			Source:      fmt.Sprintf("Service-%s", metrics.ID),
