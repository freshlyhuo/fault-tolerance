...
func main() {
	...
	scenario := flag.String("scenario", "", "当前测试场景，用于输出 P-001 至 P-005 场景级性能结果")
	flag.Parse()
	...
	perf := newScenarioPerfTracker(*scenario)
	alertByBasicEvent, err := loadAlertByBasicEvent(*diagnosisConfigPath)
	if err != nil {
		fmt.Fprintf(os.Stderr, "加载故障树告警映射失败: %v\n", err)
		os.Exit(1)
	}
	perf.SetAlertByBasicEvent(alertByBasicEvent)
	...
	recoverySvc := recovery.NewRecoveryServiceWithHealthMonitorState(
		...
		recovery.RecoveryServiceConfig{
			QueueSize:    *recoveryQueueSize,
			OnPlanStart:  perf.ObserveRecoveryPlanStart,
			OnPlanFinish: perf.ObserveRecoveryPlanFinish,
		},
	)
...
func loadAlertByBasicEvent(configPath string) (map[string]string, error) {
	loader := diagnosisConfig.NewLoader(configPath)
	faultTrees, err := loader.LoadFaultTrees()
	if err != nil {
		return nil, fmt.Errorf("加载故障树配置失败: %w", err)
	...
	for _, faultTree := range faultTrees {
		if faultTree == nil {
			continue
		}
		for _, basicEvent := range faultTree.BasicEvents {
			if basicEvent.EventID == "" || basicEvent.AlertID == "" {
				continue
			}
			out[basicEvent.EventID] = basicEvent.AlertID
		}
	}
	return out, nil
}
